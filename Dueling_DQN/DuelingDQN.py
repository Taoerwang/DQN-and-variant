...
class Dueling_DQN(object):
    ...
    def build_net(self, s, scope, trainable):
        with tf.variable_scope(scope):
            ...
            v_z = tf.layers.dense(x, 1, trainable=trainable)
            a_z = tf.layers.dense(x, self.a_dim, trainable=trainable)
            # make the sum(a_z) = 0
            q_z = v_z + (a_z - tf.reduce_mean(a_z))
        return q_z
    ...
    def build_model(self, lr):
        ...
        # argmax(Q) following code is double dqn structure
        max_a = tf.argmax(q_eval_z, axis=1)
        one_hot_max_a = tf.one_hot(max_a, self.a_dim)

        # y = R + gamma * Q_(S, argmax(Q))
        q_target = reward + self.gamma \
                   * tf.reduce_sum(one_hot_max_a * q_target_z, axis=1, keepdims=True) * (1 - done)
    ...
    def play(self):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(100):
                ...
                while True:
                    # self.env.render()
                    a = self.choose_action(s, sess, q_eval_z, state, epsilon=0)
                    s_, r, d, _ = self.env.step(a)
...
if __name__ == '__main__':
    agent = Dueling_DQN(batch_size=128, gamma=0.99, lr=0.01, epsilon=0.1, replace_target_iter=300)
    # agent.train()
